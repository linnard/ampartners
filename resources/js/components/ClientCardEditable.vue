    ...
    <div class="wrap-parent-client">
        ...
        <div class="Receipt Receipts_item" data-id="">
            ...
            <div class="ReceiptForm Receipt_form">
                <div class="ReceiptForm_column ReceiptForm_column-fio">
                    <div class="ReceiptForm_itemGroup">
                        <div class="ReceiptForm_item">
                            ...
                            <input class="FormText ReceiptForm_text" :class="{'invalid':err.lastname}" type="text" v-model="client.lastname"
                                   @change="inputChanged" :disabled="(!editable)" name="lastname">
                        </div>
                        <div class="ReceiptForm_item">
                            <label class="FormLabel ReceiptForm_label">Ім’я (англ.)</label>
                            <input class="FormText ReceiptForm_text" :class="{'invalid':err.firstname}" type="text" v-model="client.firstname"
                    ...
                    <div class="ReceiptForm_itemGroups">
                        <div class="ReceiptForm_itemGroup ReceiptForm_itemGroup-communicationTypes">
                            <div class="ReceiptForm_item">
                                ...
                                <select class="Select Select-receiptForm" :disabled="(!editable)" v-model="client.properties[0].type_id"
                                        @change="updateProps" :class="{'invalid':err.propType}" @focus="err.propType = false">
                            ...
                            <div class="ReceiptForm_item">
                                ...
                                <input class="FormText ReceiptForm_text" type="text" v-model="client.properties[0].value"
                                       @change="updateProps" :disabled="(!editable)" @focus="err.propVal = false" :class="{'invalid':err.propVal}">
                            </div>
                        </div>
                        <div class="ReceiptForm_itemGroup ReceiptForm_itemGroup-communicationTypes">
                            <div class="ReceiptForm_item">
                                <label class="FormLabel ReceiptForm_label">Додатковий тип зв’язку</label>
                                <select class="Select Select-receiptForm" :disabled="(!editable)" v-model="client.properties[1].type_id"
                                        @change="updateProps">
                                    <option value="0" disabled selected>Не вибрано</option>
                                    <option v-for="type in property_types" :value="type.id">
                                        {{type.name}}
                            ...
                            <div class="ReceiptForm_item">
                                <label class="FormLabel ReceiptForm_label">Деталі зв’язку</label>
                                <input class="FormText ReceiptForm_text" :disabled="(!editable)" type="text" v-model="client.properties[1].value"
                                       @change="updateProps">
                            </div>
                        </div>
                    </div>
                ...
                <div class="ReceiptForm_column ReceiptForm_column-vacancyInfo">
                    <div class="ReceiptForm_itemGroup ReceiptForm_itemGroup-paymentType" style="display: block;">
                        <div class="ReceiptForm_item" style="margin-right: 0">
                            ...
                            <select name="vacancy_id" class="Select Select-receiptForm" :disabled="(!editable)" :class="{'invalid':err.vacancy}" @focus="err.vacancy = false" v-model="client.vacancy_id" @change="inputChanged">
                                <option value="0" disabled selected>Не вибрано</option>
                ...
                <div class="ReceiptForm_column ReceiptForm_column-additionalForm" v-if="(['creating', 'booking_confirmation_rejected'].includes(client.status))">
                    <button @click="book" class="BtnOutline BtnOutline-transfer ReceiptForm_transfer">Забронювати</button>
                </div>

                <div class="ReceiptForm_column ReceiptForm_column-statusBlock" v-else-if="(['booking_confirmation_expected'].includes(client.status))">
                    <div class="StatusBlock">
                        <svg class="SvgIco StatusBlock_icon" viewBox="0 0 64 64" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"><g id="Icon-Clock" transform="translate(378 428)"><path class="SvgIco_path" d="M-346.1-373.1c-12.6 0-22.9-10.3-22.9-22.9 0-12.6 10.3-22.9 22.9-22.9 12.6 0 22.9 10.3 22.9 22.9.1 12.6-10.2 22.9-22.9 22.9zm0-43.4c-11.3 0-20.4 9.2-20.4 20.4s9.2 20.4 20.4 20.4 20.4-9.2 20.4-20.4-9.1-20.4-20.4-20.4z" id="Fill-131"></path><path class="SvgIco_path" id="Fill-132" d="M-338.9-384.3l-8.5-8.5v-15.5h2.7v14.4l7.7 7.7-1.9 1.9"></path></g></svg>
                        <div class="StatusBlock_text">Контакт очікує <br>підтвердження бронювання</div>
                    </div>
                </div>

                <div class="ReceiptForm_column ReceiptForm_column-statusBlock" v-else-if="(['ticket_expected', 'ticket_confirmation_rejected'].includes(client.status) && !client.ticket_uploaded)">
                    <div class="StatusBlock StatusBlock-uploadTicketPhoto">
                        <svg class="SvgIco StatusBlock_icon" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><g class="SvgIco_stroke" clip-rule="evenodd" fill-rule="evenodd" image-rendering="optimizeQuality" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" stroke="#000"><path class="fil0 str0" fill="none" stroke-width="2.000064" stroke-linecap="round" stroke-linejoin="round" d="M1.626 13.307L33.22 4.433l11.006 39.185-31.594 8.873z"></path><path class="fil0 str0" fill="none" stroke-width="2.000064" stroke-linecap="round" stroke-linejoin="round" d="M41.543 15.696l20.831 3.936-7.776 39.935-30.143-5.856"></path><path class="fil0 str0" fill="none" stroke-width="2.00016" stroke-linecap="round" stroke-linejoin="round" d="M9.768 42.383l6.816-9.023 7.007 2.784 5.76-11.04L40.68 30.96"></path><path class="fil0 str1" d="M19.175 22.896c1.056-.288 2.112.288 2.4 1.344.288 1.056-.288 2.112-1.344 2.4-1.056.288-2.112-.288-2.4-1.344-.288-1.056.288-2.112 1.344-2.4zM11.208 48.24l32.063-9.024" fill="none" stroke-width="2.00016"></path><path class="fil0 str0" fill="none" stroke-width="2.00016" stroke-linecap="round" stroke-linejoin="round" d="M36.839 50.255l14.207 2.976M42.79 22.32l14.208 2.976"></path></g></svg>
                        <h4 class="StatusBlock_title">{{client.latest_log.comment}}</h4>
                        <div class="StatusBlock_text">Для отримання маршрутного листа та даних для зв'язку з координатором - завантажте фото квитка на автобус або поїзд</div>
                    </div>
                </div>


                <div class="ReceiptForm_column ReceiptForm_column-statusBlock  ReceiptForm_column-transparent" v-else-if="(['ticket_expected', 'ticket_confirmation_rejected'].includes(client.status) && client.ticket_uploaded)">
                    <div class="StatusBlock StatusBlock-ticketLoaded">
                        <button @click="ticketUploaded" class="BtnOutline BtnOutline-transfer StatusBlock_btn">Квиток завантажено</button>
                        <div class="StatusBlock_text">{{client.latest_log.comment}}</div>
                    </div>
                </div>

                <div class="ReceiptForm_column ReceiptForm_column-statusBlock" v-else-if="(['ticket_confirmation_expected'].includes(client.status))">
                    <div class="StatusBlock">
                        <svg class="SvgIco StatusBlock_icon" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path class="SvgIco_path" d="M16 31C7.729 31 1 24.271 1 16c0-3.646 1.323-7.16 3.727-9.895a.502.502 0 01.706-.045.502.502 0 01.046.706A13.972 13.972 0 002 16c0 7.72 6.28 14 14 14s14-6.28 14-14S23.72 2 16 2c-2.504 0-4.96.668-7.103 1.933a.5.5 0 11-.507-.862A14.979 14.979 0 0116 1c8.271 0 15 6.729 15 15s-6.729 15-15 15z" fill="#263238"></path><path class="SvgIco_path" d="M16 28.5C9.107 28.5 3.5 22.893 3.5 16S9.107 3.5 16 3.5 28.5 9.107 28.5 16 22.893 28.5 16 28.5zm0-24C9.659 4.5 4.5 9.659 4.5 16S9.659 27.5 16 27.5 27.5 22.341 27.5 16 22.341 4.5 16 4.5z" fill="#263238"></path><path class="SvgIco_path" d="M20.75 8.27c-.13 0-.26-.05-.351-.14-.1-.1-.149-.23-.149-.36s.05-.26.149-.35c.19-.19.521-.19.7 0 .101.09.15.22.15.35s-.049.26-.149.35a.47.47 0 01-.35.15zM11.25 24.729c-.13 0-.26-.06-.351-.149a.503.503 0 01-.149-.351c0-.13.05-.26.14-.359.19-.181.53-.181.71 0 .101.1.15.229.15.359 0 .12-.05.25-.141.351a.548.548 0 01-.359.149zM24.229 11.75a.496.496 0 01-.5-.5c0-.13.051-.26.141-.351a.506.506 0 01.859.351c0 .13-.05.26-.149.35-.09.1-.221.15-.351.15zM7.77 21.25c-.13 0-.25-.05-.35-.141a.535.535 0 01-.15-.359c0-.13.05-.25.141-.351a.518.518 0 01.72 0c.09.091.14.221.14.351s-.05.26-.14.35a.495.495 0 01-.361.15zM25.5 16.5c-.13 0-.26-.05-.351-.15-.099-.09-.149-.22-.149-.35s.05-.26.149-.351a.513.513 0 01.7-.01.508.508 0 01.01.711.504.504 0 01-.359.15zM6.5 16.5c-.13 0-.26-.05-.351-.15C6.05 16.26 6 16.13 6 16s.05-.26.149-.351a.513.513 0 01.7-.01.508.508 0 01.01.711.504.504 0 01-.359.15z" fill="#263238"></path><g><path class="SvgIco_path" d="M24.229 21.25c-.13 0-.26-.05-.359-.141-.09-.1-.141-.229-.141-.359s.051-.26.141-.351a.506.506 0 01.859.351c0 .13-.05.26-.149.35-.09.1-.221.15-.351.15zM7.77 11.75c-.13 0-.25-.05-.35-.141a.535.535 0 01-.15-.359c0-.13.05-.25.141-.351a.518.518 0 01.72 0c.09.091.14.221.14.351s-.05.26-.14.35a.513.513 0 01-.361.15z" fill="#263238"></path></g><g><path class="SvgIco_path" d="M20.75 24.729c-.13 0-.26-.05-.351-.14-.1-.11-.149-.23-.149-.36s.05-.26.14-.359c.2-.19.521-.181.71 0 .101.1.15.229.15.359 0 .12-.05.25-.141.351a.502.502 0 01-.359.149zM11.25 8.27c-.13 0-.26-.05-.351-.149-.099-.091-.149-.222-.149-.351s.05-.26.149-.35a.51.51 0 01.7-.01c.101.1.15.229.15.359s-.049.261-.149.351c-.1.1-.22.15-.35.15z" fill="#263238"></path></g><g><path class="SvgIco_path" d="M16 26c-.13 0-.25-.05-.351-.141-.099-.109-.149-.229-.149-.359s.05-.26.14-.351a.506.506 0 01.71 0c.1.091.15.221.15.351s-.05.26-.15.35A.47.47 0 0116 26zM16 7c-.13 0-.25-.05-.351-.141a.507.507 0 01-.009-.71.506.506 0 01.71 0A.496.496 0 0116 7z" fill="#263238"></path></g><g fill="#263238"><path class="SvgIco_path" d="M22.364 9.636a.5.5 0 00-.707 0l-5.413 5.413a.975.975 0 01.707.707l5.413-5.413a.499.499 0 000-.707zM15.756 15.049l-6.12-6.12a.5.5 0 00-.707.707l6.12 6.12a.975.975 0 01.707-.707z"></path><path class="SvgIco_path" d="M16 17.5c-.827 0-1.5-.673-1.5-1.5s.673-1.5 1.5-1.5 1.5.673 1.5 1.5-.673 1.5-1.5 1.5zm0-2c-.275 0-.5.225-.5.5s.225.5.5.5.5-.225.5-.5-.225-.5-.5-.5z"></path></g><g><path class="SvgIco_path" d="M7.5 4.5c0 .275-.225.5-.5.5a.501.501 0 01-.5-.5c0-.275.225-.5.5-.5s.5.225.5.5z" fill="#263238"></path></g></svg>
                        <div class="StatusBlock_text">Квиток завантажено. <br>Очікується підтвердження від адміністрації</div>
                    </div>
                </div>

                <div class="ReceiptForm_column ReceiptForm_column-statusBlock ReceiptForm_column-statusBlockCompleted" v-else-if="(['completed'].includes(client.status))">
                    <div class="StatusBlock StatusBlock-completed">
                        <div class="StatusBlock_titleIcon">
                            <svg class="SvgIco StatusBlock_icon StatusBlock_icon-titleIcon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path class="SvgIco_path" d="M12 0C5.373 0 0 5.373 0 12c0 6.628 5.373 12 12 12s12-5.372 12-12c0-6.627-5.373-12-12-12zm7.754 9.561l-8.607 8.607a.448.448 0 01-.637 0l-1.272-1.285c-.175-.176-.462-.464-.636-.642l-2.96-3.112a.47.47 0 01-.133-.327c0-.115.046-.227.133-.314l1.297-1.169a.447.447 0 01.636 0l2.936 2.995a.447.447 0 00.637 0l6.699-6.681a.447.447 0 01.636 0l1.272 1.285a.458.458 0 01-.001.643z"></path></svg>
